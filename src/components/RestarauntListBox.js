...
const RestarauntListBox = (props) => {
  ...
  useEffect(() => {
    ...
    //http://localhost:5000
    Axios.get(`cors-anywhere.herokuapp.com/https://plan-buddy.herokuapp.com/api/get-restaraunt-list/${sessions_poll_id}`)
    .then((response) => {
  ...
  const updateRestarauntList = () => {
    Axios.get(`cors-anywhere.herokuapp.com/https://plan-buddy.herokuapp.com/api/get-restaraunt-list/${pollId}`)
    .then((response) => {
  ...
  const addNewRestaraunt = (restarauntName) => {
    Axios.post(`cors-anywhere.herokuapp.com/https://plan-buddy.herokuapp.com/api/add-restaraunt/`, {
      pollId: pollId,
  ...
  const upvoteRestaraunt = (restarauntName) => {
    Axios.post(`cors-anywhere.herokuapp.com/https://plan-buddy.herokuapp.com/api/upvote-restaraunt/`, {
      pollId: pollId,
      restarauntName: restarauntName
    })
    .then((response) => {
  ...
  const downvoteRestaraunt = (restarauntName) => {
    Axios.post(`cors-anywhere.herokuapp.com/https://plan-buddy.herokuapp.com/api/downvote-restaraunt/`, {
      pollId: pollId,
      restarauntName: restarauntName
    })
    .then((response) => {
